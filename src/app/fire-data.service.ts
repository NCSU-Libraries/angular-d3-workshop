...
export class FireDataService {
  ...
  // An observable is an array of data that can change.
  getData(): Observable<Fire[]> {
    return this.http.get<Fire[]>(this.fireDataUrl);
  }

  // This is the area of the application that we could put an update function (if we were using a 'live')

  // This allows the applciation to delete the selected line from the table when the 'remove' button is clicked.
